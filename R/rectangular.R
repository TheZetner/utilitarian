...
#' \donttest{
#'   library(tidyselect)
#'   df <- data.frame(name = sample(letters, 20, replace = T),
#'                month = sample(month.name, 20, replace = T),
#'             letters = sample(LETTERS[1:10], 20, replace = T),
#'             nums = floor(runif(20, 1, 15)))
#'   dplyr::count(df, name)
#'   exposeDupes(df, name)
#'   exposeDupes(df, name, listout = F)
#'   }
#' @export
